...
{ O("BEG overMonad\n"); O("    sd(a):");sd(a); O("    sd(b):");sd(b);
  ...
  P(n<0,IE)
  if(useN) //n f/x
  { I f=0; DO(n, c=dv_ex(0,p-1,u);
                if(b!=u) cd(u);
                if(f && b==c)cd(c);
                f=1; U(u=c) )
    c=c?c:ci(b); }
  else if(useB) // b f/x
       { I t; do{ K*aa=&a; fdvx=1;
