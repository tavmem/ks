...
{ O("BEG eachpair2\n"); O("    sd(a):");sd(a); O("    sd(b):");sd(b);
  ...
  if(bt<0)  DO(bn-1, h=newK(ABS(bt),1); g=newK(ABS(bt),1); memcpy(h->k,((V)b->k)+(i)*bp(bt),bp(bt));
  ...
  if(bt==0) DO(bn-1, O("~EX dv_ex(kK(b)[i+1],p-1,kK(b)[i])      K dv_ex(K a, V *p, K b) <- Z K eachpair2(K a, V *p, K b)      i: %lld      ",i);
